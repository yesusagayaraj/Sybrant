...
class Settings_ExtensionStore_ExtensionStore_View extends Settings_Vtiger_Index_View {
	...
	$invoiceList = $modelInstance->getInvoices($request);
        $invoiceHeader = $modelInstance->getInvoiceViewFields();
        
        $viewer->assign('MODULE_INST', $modelInstance);    
        $viewer->assign('DOWNLOAD_URL', $modelInstance->getExtensionsLookUpUrl());      
        $viewer->assign('LISTVIEW_HEADERS', $invoiceHeader); 
        $viewer->assign('LISTVIEW_ENTRIES', $invoiceList);	
    ...
    protected function installationLog(Vtiger_Request $request) {
        ...
        if ($extensionId && $extensionModel) {
           ...
           } else  if (strtolower($moduleType) === 'layout') {
