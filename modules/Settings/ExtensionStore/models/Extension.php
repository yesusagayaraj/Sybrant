...
class Settings_ExtensionStore_Extension_Model extends Vtiger_Base_Model {
	...
	var $listFields = Array(
			'name'=> 'LBL_PAID_FOR',
            'price'=> 'LBL_AMOUNT', 
            'chargedon' => 'LBL_PAID_ON' ,
            'paymentstatus' => 'LBL_PAYMENT_STATUS',
            'download' => 'LBL_DOWNLOAD',
            );
     
	public function getInvoiceViewFields() {
	...
	public function isAlreadyExists() {
		...
		if($moduleModel) {
            ...
            return true;
        }else if(self::getLayoutInstance($moduleName)){
            return true;
        }
		return false;
	}
    
    public static function getLayoutInstance($lang) {
        $sql = 'SELECT id,name,label FROM vtiger_layout WHERE name = ?';
	...
	public function getInstanceFromArray($listing) {
        ...
        if($moduleModel && $moduleModel->get('extnType') == 'language' && $moduleModel->get('extnType') == 'layout' ) {
    ...
    public static function getModuleFromExtnName($extnName) {
        ...
        if(!$moduleModel) {
            ...
            else if(self::getLayoutInstance($extnName)) {
               $moduleModel = new Vtiger_Module_Model();
               $moduleModel->set('name',$extnName);
               $moduleModel->set('isentitytype',false);
               $moduleModel->set('extnType','layout');
            }
        }
        return $moduleModel;
    }
	...
	public function getInstanceFromScreenShotArray($listing) {
       ...
       public function getCardDetails($cardId) {
        ...
        public function getLocationUrl($extensionId, $extensionName){
         ...
         /**
         * Function to get extension store  users invoice
         */
          public function getInvoices($request) {
            $extensionLookUpUrl = $this->getExtensionsLookUpUrl();
            if ($extensionLookUpUrl) {
                    $connector = Settings_ExtensionStore_ExtnStore_Connector::getInstance($extensionLookUpUrl);
                    $response = $connector->getInvoices();
                    return $response;
            }
        }
         
        /**
         * Function to convert the Date from  marketplace.vtiger.com to user format
          * 
          *  @param <String> $dateTime
          * 
         */
       public function getDisplayValueofDate($dateTime) {
            $dateTime= new DateTime($dateTime);
            return Vtiger_Datetime_UIType::getDisplayValue($dateTime->format('Y-m-d H:i:s'));
         }
}
