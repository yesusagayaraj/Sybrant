...
<!--Project Details
    Date :14-12-2016
    -->
{if $FIELD_MODEL->getFieldName() eq 'product_id' && $MODULE eq 'HelpDesk'} 
<input id="sourceRecord" type="hidden" name="sourceRecord" value="{$SOURCE_RECORD}" />
<input id="record" type="hidden" name="record" value="{$RECORD_ID}" />
<select class="chzn-select {if $OCCUPY_COMPLETE_WIDTH} row-fluid {/if}" id="project_id" name="project_id" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" data-fieldinfo='{$FIELD_INFO|escape}' {if !empty($SPECIAL_VALIDATOR)}data-validator='{Zend_Json::encode($SPECIAL_VALIDATOR)}'{/if} data-selected-value=''>
		{if $FIELD_MODEL->isEmptyPicklistOptionAllowed()}<option value="">{vtranslate('LBL_SELECT_OPTION','Vtiger')}</option>{/if}
	{foreach key=PROJECT_ID item=PROJECT_NAME from=$PROJECT_DETAILS}

	<option value="{$PROJECT_ID}" data-picklistvalue= '{$PROJECT_NAME}' {if $PROJECTID eq $PROJECT_ID} selected {/if}
						{if array_key_exists($PROJECTID, $PROJECT_DETAILS)} data-recordaccess=true {else} data-recordaccess=false {/if}
