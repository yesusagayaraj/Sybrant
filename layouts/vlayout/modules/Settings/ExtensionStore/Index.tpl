...
{strip}
        ...
        <div class="row-fluid">
            ...
            <span class="span6">
                <span class="pull-right">
                    ...
                    {elseif $IS_PRO}
                        <button id="getInvoices"   class="btn btn-primary">{vtranslate('LBL_INVOICES', $QUALIFIED_MODULE)}</button>&nbsp;
 ...
 <!-- invoice detals start-->   
 <div class="modal invoicelistModal hide">
     ...
     {if empty($LISTVIEW_ENTRIES)}
         <div class="alert">
             {vtranslate('LBL_NOT_PURCHASED_YET', $QUALIFIED_MODULE)}
         </div>
     {else}
             ...
             <table class="table table-bordered  table-condensed listViewEntriesTable">
                 <thead>
                     <tr class="listViewHeaders">
                         {foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
                             <th nowrap>
                                 <a  class="listViewHeaderValues">{vtranslate($LISTVIEW_HEADER,$QUALIFIED_MODULE)}</a>
                             </th>
                         {/foreach}
                     </tr>
                 </thead
                 <tbody>
                     {foreach item=LISTVIEW_ENTRY from=$LISTVIEW_ENTRIES}
                         <tr class="listViewEntries"  data-id={$LISTVIEW_ENTRY['id']}  >
                     <div class="pull-right actions">
                         {foreach item=LISTVIEW_HEADER key=HEADERKEY from=$LISTVIEW_HEADERS}
                             {if $HEADERKEY eq 'name' }
                                 <td class="listViewEntryValue">{$LISTVIEW_ENTRY['listing'][$HEADERKEY]}</td>
                             {elseif $HEADERKEY eq 'price'   }  
                                 <td class="listViewEntryValue">$ {$LISTVIEW_ENTRY[$HEADERKEY]}</td>
                             {elseif $HEADERKEY eq 'chargedon'   }  
                                 <td class="listViewEntryValue">{$MODULE_INST->getDisplayValueofDate({$LISTVIEW_ENTRY[$HEADERKEY]})}</td>
                             {elseif $HEADERKEY eq 'download'   }  
                                 <td><a class="text-warning"  href="{$DOWNLOAD_URL}/customer/mysubscriptions?id={$LISTVIEW_ENTRY['id']}&amp;type=pdf" target="_blank">{vtranslate('LBL_PDF', $QUALIFIED_MODULE)}</a> </td>
                             {else}   
                                 <td class="listViewEntryValue">{$LISTVIEW_ENTRY[$HEADERKEY]}</td>
                             {/if}
                         {/foreach}
     ...
     <div class="modal-footer">
         <div class="row-fluid">
             <div class="pull-right">
                 <button class="btn btn-success "  type="reset" data-dismiss="modal" ><strong>{vtranslate('LBL_OK', $QUALIFIED_MODULE)}</strong></button>
             </div>
         </div>
     </div>
 {/if}
