...
jQuery.Class('Settings_ExtensionStore_Js', {
    ...
    registerEventsForExtensionStore: function(container) {
     ...
     jQuery(container).on('click', '#logoutMarketPlace', function(e) {
  	...
  	jQuery(container).on('click', '#getInvoices', function(e) {
            var element = jQuery(e.currentTarget);
            var invoicelistModal = jQuery(container).find('.invoicelistModal').clone(true, true);
            invoicelistModal.removeClass('hide');
            var progressIndicatorElement = jQuery.progressIndicator();
              app.showModalWindow(invoicelistModal, function(data) {
                progressIndicatorElement.progressIndicator({'mode': 'hide'});
                 }, {'width': '1000px'});
        });
